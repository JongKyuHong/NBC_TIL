...
- 메모리에서 언로드할때 지연시간 가지기

## 배운거

- 데이터 테이블과 데이터 에셋
  데이터 테이블로도 데이터 에셋이 할수있는 일을 대체해서 사용할 수 있다.
  데이터테이블에서 TSoftObjectPtr같이 경로만 들고있을수 있음 문자열로
  굳이 치면 단점은 데이터 테이블은 한번에 메모리에 다 올려야 한다. 그래서 데이터 테이블 자체는 하드로 사용

- magic number

매직 넘버(Magic Number)란 코드 안에서 의미 설명 없이 그냥 하드코딩된 숫자(또는 문자열) 상수
