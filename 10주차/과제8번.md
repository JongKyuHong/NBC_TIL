![](https://velog.velcdn.com/images/kyu_/post/8a513ae3-3711-4a19-a4fa-adfc0fa30670/image.png)
...
- [x] 멀티 웨이브 구현
  ...
  - [x] 각 웨이브는 시간과 아이템 갯수 다르게
  - [x] 웨이브 시작시 "Wave 시작!" 알림 출력

- [x] UI 리뉴얼
  - [x] 점수, 시간, 체력 전부 한 화면에 나오게
  - [x] 메인 메뉴 (시작, 종료) 재설계
  - [x] 게임 오버 메뉴 (재시작, 메인 메뉴 돌아가기)

## 도전

- [x] 부정적 아이템 추가하기 (슬로우, 키반전, 시야제한 등)
  - [x] 아이템 효과 중첩 가능하게
  - [x] UI로 각 디버프정보 확인

- [x] 웨이브 별 환경 변화 일으키기
  - [x] 이벤트 발생을 플레이어가 인지할 수 있도록 UI로 출력하기

- [x] UI애니메이션 1개이상 구현하기
  - [x] Widget Animation으로 Fade In/Out, 슬라이드 적용
  - [x] 버튼 hover상태일때 Tween또는 강조 효과 넣어보기
...
- 레벨업하면 증강이 나오는데 그때 게임오버 화면이 같이나오는 현상이 벌어졌다.
- 범인은 캐릭터에 있었다.
...
void AMyCharacter::BeginPlay()
{
	...
	if (AbilitySystemComponent)
	{
		...
		AbilitySystemComponent->GetGameplayAttributeValueChangeDelegate(
			AttributeSet->GetExpAttribute()).AddUObject(this, &AMyCharacter::OnHealthChanged);
	}
	UpdateOverheadBar();
}
...
- 현재는 OverHeadBar에서 경험치와 체력을 둘다 관리하고 있고, 경험치가 0이되었을때 마치 체력이 0이된것처럼 작동해서 그런거였다.
- UpdateOverheadBar를 분리하는게 더 맞을거같지만 우선은 부르는 함수를 분리하였다.
